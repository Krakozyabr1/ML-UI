...
with left:
    with st.form("file_selector_form", clear_on_submit=False):
        ...
        saveto_name = st.text_input("Select output .csv file name:", value="ecg_features").replace('.csv', "")
        if learning_check:
        ...
        wlt = st.selectbox("Select wavelet:", index=3, options=[
                            'haar',
        ...
        wlt_level = int(st.text_input("Level:", value="4"))
